...
3) well-wishes to to those who accompanied us during our promenade around the sun.  
...
Numbers for global R-Ladies chapters are shown in a way to highlight how we organize ourselves. We also show what was done and how much we grew in 2019. To be able to express that idea we chose _the number of chapters, the number of countries, the number of members, and the total events held in the year 2019_, because this are concrete indicators. The first three numbers were extracted from the [R-Ladies Community Board](https://benubah.github.io/r-community-explorer/rladies.html). The amount of events was calculated using the [meetupr](https://github.com/rladies/meetupr) package, developed by R-Ladies.

To follow our steps, the first thing is to load the necessary packages and generate the necessary variables and functions:

```
# API KEY of Meetup
Sys.setenv(MEETUP_KEY = "your API Key") # just a small comment from me: the API keys is soon deprecated, maybe add a small notice on that?
...
# Code by Jenny Bryan
# Another small comment by me: you are already loading purrr, you could use purrr::slowly here instead?
...
```
# We get all the events already done.

eventos <- rladies_groups$urlname %>%
  map(slowly(safely(get_events)), event_status='past') %>% transpose()

# In events we have a list with all the event data: name, date,
...
# We were only interested in the information we had in results. So we keep that information
...
# We create a logical vector with events where there is error
...
# Get all twitter users that use the word R Ladies
...
# We calculate the total followers of all accounts
rladies %>% 
...
Here we wanted to wish a happy year for all R-Ladies, and also for all the allies who accompanied us during the year. The selected image is our logo and the address of our website.

The text that accompanies the scene is:
...
R-Ladies is a global community, the video had to be in English: the language the world speaks. But why not also in Spanish? The R Latin-American Community has grown a lot in this time and in large part it has been thanks to the effort and work of the R-Ladies in this region of the world, so we decided to generate the video in both languages to celebrate this hard work. [Laura Acion](https://twitter.com/_lacion_/) was responsible for correcting and translating the text of each scene.

### Text, images ... audio?

Now, a video with only letters, numbers and images would leave many people out of our message, so we decided to record the audio of the message. For that we got the help of the genius of [Alejandra Bellini](https://twitter.com/AlejaBellini), who recorded the audio in Spanish and English. She recorded it using WhatsApp with a cell phone, then we used [Zamzar](https://www.zamzar.com) to transform the audio to MP3 and [Mp3cut](https://mp3cut.net/en/) to cut that audio in the parts necessary to be able to synchronize the audio with the text and the video images.


## Third step: editing ...

_Third step_: with the plan in mind, it was time to edit the video. We did it using Doodly for the effects of the drawing hand . Both music and fonts are provided by the software. The most laborious part was the synchronization of the audio with the drawing of the numbers and letters.

The result was two videos in Spanish and English, where we tell in a minute what R-Ladies did during 2019. It was a very fun task, with many laughs and attempts, especially recording the text in English.
