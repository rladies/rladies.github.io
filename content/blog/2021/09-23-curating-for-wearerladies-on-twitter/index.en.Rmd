...
tweetrmd::include_tweet(
  "https://twitter.com/WeAreRLadies/status/1361660625718571010"
)
```
...
```{r}
timeline <- read_csv(here::here(
  "content",
  "post",
  folder,
  "data",
  "rladies_timeline.csv"
)) %>%
  mutate(
    even_odd = (id %% 2) == 0,
...
timeline %>%
  dplyr::select(
    `Time before curation` = time_frame,
    `Action taken` = action
  ) %>%
  knitr::kable()
```
...
polls <- read_csv(here::here(
  ...
  mutate(
    ...
    tweet_label = glue::glue("Tweet #{step}") %>%
      fct_reorder(step) %>%
      fct_rev(),
